...
#'   shift_by = 1
#' )
determine_split_regions <- function(regions_bed, genes_gft, limit = 2^29, shift_by = 1L) {
	cli::cli_alert_info(
...
fix_split_boundaries <- function(split_df, chr_genes, limit, shift_by = 1L) {
	if (nrow(split_df) <= 1L || nrow(chr_genes) == 0L) return(split_df)
	...
	repeat {
		...
		for (i in seq_along(boundaries)) {
			...
			repeat {
				...
				if (b_new >= chr_end || b_new <= b) break
				b <- b_new
			}
			boundaries[i] <- b
		}
		...
		for (i in seq_len(length(all_points) - 1L)) {
			...
			if ((seg_end - seg_start) > limit) {
				...
				inner <- as.integer(round(seq(seg_start + sub_chunk_size, seg_end - sub_chunk_size, by = sub_chunk_size)))
