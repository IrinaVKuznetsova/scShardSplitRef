...
# FUNCTION-0 Convert BED file to a format suitable for splitting GTF files
# df,contig_prefix = "CAJHDD",mt_name = "Mt",pt_name = "Pt"
prep_centromere_BED = function(df, contig_prefix, mt_name, pt_name) {
  df_result = df %>%
    rowwise() %>%
    do({chr = .$Chr
      start = .$CentrStart
...
# FUNCTION-1 check if GTF file has overlapping gene features with centromere coordinates and prepare new split coordinates if thats the case
prepare_centromere_split = function(gtf_path, centromere_path, out_path, contig_prefix, mt_name, pt_name) {
  ...
  # --------------------------------------------------
#gtf_path = "/data/02_PROJECTS/sc_ShardSplitRef/01_Working_R_scripts/Toy_validate_GTF_script/toy_input/A0_toy_all_scenarios_2chr.gtf"     ###/A0_toy_all_scenarios.gtf",
...
#out_path = "/data/02_PROJECTS/sc_ShardSplitRef/01_Working_R_scripts/Toy_validate_GTF_script/toy_output/"  
  gtf_file = read.table(file = gtf_path, header = FALSE, sep = "\t")
  colnames(gtf_file) = c("Chr", "source", "feature", "start", "end", "score", "strand", "frame", "attribute")
  ...
  orig_For_FASTA = prep_centromere_BED(read_cent, contig_prefix, mt_name, pt_name)
  write.table(orig_For_FASTA,
  ...
  if (nrow(check_genefeat_centr_overlap) > 0){
    message( paste("\nOverlapping gene features with centromere coordinates detected!\n",
      ...
      "Use:\n",
      out_path, "RECAL1_FINAL_SPLIT_coord_for_GTF.bed as the input to modify GTF file.\n", 
      "and:\n", 
      out_path, "RECAL1_FINAL_SPLIT_coord_for_FASTA.bed", "\n as the input for splitting FASTA.", sep="") )
      
    # 5.1 Shift centromere split coordinate 10 bp downstream of the gene feature end coordinate
    new_split_coord_for_overlap_genes_with_centrom = check_genefeat_centr_overlap$end + 10
  ...
  df_exact_centromere_split =read_cent[read_cent$Chr %in% BEDnew_split_coord_for_overlap_genes_with_centrom$Chr == FALSE, ]
  # 6.2 Combine
  BED_df_combined = rbind(df_exact_centromere_split, BEDnew_split_coord_for_overlap_genes_with_centrom)
  # 6.3 Sort as original bed file
  BED_df_combined_sorted_as_original =BED_df_combined[match(read_cent$Chr, BED_df_combined$Chr), ]

  # for GTF
  write.table(BED_df_combined_sorted_as_original,
              file = paste0(out_path, "RECAL1_FINAL_SPLIT_coord_for_GTF.bed"),
              quote = FALSE, row.names = FALSE, col.names = FALSE, sep = "\t")
  # for FASTA
  write.table(prep_centromere_BED(BED_df_combined_sorted_as_original, contig_prefix, mt_name, pt_name),
  ...
  message( paste(" \n",
                ...
                "Use:\n",
                centromere_path, " as the input to modify GTF file.\n",
                "and:\n",
                out_path, "IN0_original_centromeres_for_FASTA.bed", " for splitting FASTA.", sep=""))
    }
  }
