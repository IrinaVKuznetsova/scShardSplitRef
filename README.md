...
536.8?Mb (2?? bases). *scShardSplitRef* automates this splitting
...
error:  
\>*?Due to limitations of the BAM index format, a contig in the
reference FASTA file cannot exceed *536.8?Mb (2^29 bases)*. If a contig
exceeds that size you will have to split it into smaller contigs and
make corresponding modifications to the GTF file.?* [Source: 10x
Genomics
...
[How to modify GTF using scShardSplitRef](vignettes/my-vignette.Rmd)
...
stringr(v.1.5.1), utils(v.4.4.3) libraries. *\[remove tidyverse(v.2.0.0)
after deploying r package\]*

## **Limitations / Notes**

Please note that this package has been developed for use with diploid
organisms. What the tool does not do. \* Does not validate biological
correctness of annotations \* Does not merge or reorder GTF entries \*
Does not handle circular genomes

## **Citation**

Please cite this tool if you use it Irina Kuznetsova, Luke Pembleton,
Pasquale Luca Curci, Adam Sparks Curtin University lpembleton CNR
Institute of Biosciences and BioResources Curtin University
