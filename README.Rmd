...
## **scShardSplitRef**  
is designed for researchers working with single-cell data from species that require building a custom reference genome, particularly when one or more contigs in the reference FASTA exceed the Cell Ranger limit of 536.8?Mb (2?? bases). *scShardSplitRef* automates this splitting process, generating contig fragments that comply with *Cell Ranger* requirements. The software outputs FASTA and GTF files containing the split contigs, which can be directly processed by <code>cellranger-arc mkref</code>.
...
For example, in large genomes such as barley, several chromosomes (*2H?=?665.59?Mb, 3H?=?621.52?Mb, 4H?=?610.33?Mb, 5H?=?588.22?Mb, 6H?=?561.79?Mb, 7H?=?632.54?Mb*) exceed this size threshold. These contigs cannot be processed by `cellranger-arc mkref`. As a result, `cellranger-arc mkref` cannot build a reference unless "oversized" chromosomes are divided into smaller fragments.  
...
* Splits contigs exceeding the 536.8?Mb Cell Ranger ARC limit into valid fragments in FASTA  
...
## **Example Workflow**  
A toy example workflow walks through the [step-by-step using toy files](example).  
...
Author's list: Irina Kuznetsova, Luke Pembleton, Pasquale Luca Curci, Adam Sparks  
Curtin University, lpembleton, CNR Institute of Biosciences and BioResources, Curtin University
